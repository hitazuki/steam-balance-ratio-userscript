...
在“此物品不在货架上”提示框正上方增加一行，显示目标货币的最低含费价，并按当前
钱包手续费反推税后估算价和挂刀比例。该结果会标记为参考值，卖单恢复后仍以实际
挂单为准。若 `priceoverview` 暂时不返回最低售价，脚本会继续读取 Steam 订单深度中
的最低卖单；接口仍不可用时，才使用已上架列表中 SIH 显示并在本地保存的
`MIN.PRICE`，配合最近一次 Steam 换算率生成参考值。
